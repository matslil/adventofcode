...
fn find_antinodes(size:( usize,  usize), pair: ((usize, usize), (usize, usize))) -> Vec<(isize, isize)> {
    let dist = ((pair.0.0 as isize - pair.1.0 as isize).abs(), (pair.0.1 as isize - pair.1.1 as isize).abs());
    let antinodes = (
        (if pair.0.0 < pair.1.0 { pair.0.0 as isize - dist.0 } else { pair.0.0 as isize + dist.0 },
         if pair.0.1 < pair.1.1 { pair.0.1 as isize - dist.1 } else { pair.0.1 as isize + dist.1 }),
        (if pair.0.0 < pair.1.0 { pair.1.0 as isize + dist.0 } else { pair.1.0 as isize - dist.0 },
         if pair.0.1 < pair.1.1 { pair.1.1 as isize + dist.1 } else { pair.1.1 as isize - dist.1 }));

    let _span = tracing::span!(tracing::Level::INFO, "nr_antinodes", "{:?} => {:?}", pair, antinodes).entered();

    let mut result: Vec<(isize, isize)> = Vec::new();

    if valid_node(size, antinodes.0) {
        result.push(antinodes.0);
    }
    if valid_node(size, antinodes.1) {
        result.push(antinodes.1);
    }
    result
}
...
fn test() {
    setup_tracing();
    assert_eq!(14, get_answer("test.a"));
}
