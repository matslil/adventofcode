...
#[instrument(skip(towels, display))]
fn can_display(towels: &HashMap<char, Vec<Vec<char>>>, display: &Vec<char>, idx: usize) -> bool {
    if idx >= display.len() {
        info!("Match!");
        return true;
    }
    ...
    if let Some(towel_list) = towels.get(&display[idx]) {
        ...
        for towel in towel_list {
            ...
            debug!("Try: {:?}", towel);
            if can_display(towels, display, idx + towel.len()) {
                return true;
            }
        }
    }
...
fn get_answer(file: &str) -> usize {
    ...
    for gr_display in displays {
        ...
        if can_display(&towel_map, &gr_display, 0) {
            result += 1;
        }
    }
    return result;
